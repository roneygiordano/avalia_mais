...
def renderizar_tela_cadastro():
    ...
    # --- Bloco Novo: Lista de Pacientes Compacta no Topo ---
    # Colocamos um Expander que fica fechado por padrão. Não empurra nada para baixo!
    with st.expander("👥 Visualizar Lista de Pacientes Cadastrados (Clique para abrir/fechar)", expanded=False):
        if todos_pacientes:
            # Converte a lista de tuplas em um DataFrame do Pandas para exibir uma tabela linda
            df_pacientes = pd.DataFrame(todos_pacientes, columns=["ID", "Nome do Paciente", "Data de Nascimento", "Dias de Atendimento"])
            # Remove a coluna de índice padrão do Pandas para ficar mais limpo
    ...
    # --- CONFIGURAÇÃO DOS VALORES PADRÃO PARA CADASTRO/EDIÇÃO ---
    ...
    if acao == "Editar Paciente Existente":
        opcoes_edicao = {f"{p[1]} (ID: {p[0]})": p for p in todos_pacientes}
        paciente_selecionado = st.selectbox("Selecione o paciente que deseja modificar:", list(opcoes_edicao.keys()))
        
        p_dados = opcoes_edicao[paciente_selecionado]
        id_paciente_edicao = p_dados[0]
        nome_padrao = p_dados[1]
        
        try:
            data_padrao = datetime.strptime(p_dados[2].split(" ")[0], "%Y-%m-%d").date()
        except:
            try:
                data_padrao = datetime.strptime(p_dados[2], "%d/%m/%Y").date()
            except:
                data_padrao = date(1950, 1, 1)
                
        dias_padrao_idx = 1 if p_dados[3] == "Terça e Quinta" else 0
